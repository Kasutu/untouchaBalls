...
export function enemyHandler(arr: Pokemon[], playerInBattle: Pokemon) {
  const enemyAttrib: EnemyAttrib[] = [
    {
      name: 'enemy1',
  ...
  enemyAttrib.forEach((e: EnemyAttrib) => {
    const tempEnemy: Enemy = new Enemy(e.name, e.hp, e.damage);
    tempEnemy.x = e.x;
    tempEnemy.y = e.y;
